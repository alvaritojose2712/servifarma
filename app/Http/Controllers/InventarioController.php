    ...
    public function setCarrito(Request $req)
    {
        try {
            ...
            if (isset($numero_factura)) {
                ...
                $q_ct = inventario::select(["cantidad"])->find($id_producto)->cantidad;

                if ($cantidad>$q_ct) {
                    throw new \Exception("¡No hay la cantidad que solicita!", 1);
                }
            ...
            return Response::json(["msj"=>$e->getMessage(),"estado"=>false,"num_pedido"=>"0","type"=>"agregar"]);
        }
