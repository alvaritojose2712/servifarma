...
{   
    ...
    protected $sends = [
        ...
        "lilisebastian2014@gmail.com",
       // "melysalazar192@gmail.com",          
    ];
    protected  $letras = [
                1=>"L",
                2=>"R",
    ...
    public function sendinventario()
    {
        ...
        $d = [
            ...
            $d["inv"]["data"] = $data;
            $d["sucursal"] = $sucursal;
            $d["inv"]["categorias"] = $categorias;
            $d["inv"]["proveedores"] = $proveedores;

            $d["inv"]["descripcion"] = $descripcion;
            $d["inv"]["precio_base"] = $precio_base;
            $d["inv"]["precio"] = $precio;
            $d["inv"]["cantidad"] = $cantidad;
            $d["inv"]["proveedor"] = $proveedor;
            $d["inv"]["categoria"] = $categoria;
            $d["inv"]["marca"] = $marca;

            $d["inv"]["count"]  =  count($data);
            $d["inv"]["costo"]  =  number_format($costo,"2");
            $d["inv"]["venta"]  =  number_format($venta,"2");

            $d["inv"]["view_codigo_proveedor"]  =  true;
            $d["inv"]["view_codigo_barras"]  =  true;
            $d["inv"]["view_descripcion"]  =  true;
            $d["inv"]["view_proveedor"]  =  false;
            $d["inv"]["view_categoria"]  =  false;
            $d["inv"]["view_id_marca"]  =  false;
            $d["inv"]["view_cantidad"]  =  true;
            $d["inv"]["view_precio_base"]  =  true;
            $d["inv"]["view_t_costo"]  =  true;
            $d["inv"]["view_precio"]  =  true;
            $d["inv"]["view_t_venta"]  =  true;


        $from1 = $sucursal->correo;
        $from = $sucursal->sucursal;
        $subject = $sucursal->sucursal." | INVENTARIO DIARIO | ".$this->today();
        //return view("reportes.inventariosend")->with($d);
        try {
            Mail::to($this->sends)->send(new enviarInventario($d,$from1,$from,$subject));    
            
            return Response::json(["msj"=>"INVENTARIO enviado con Éxito","estado"=>true]);
